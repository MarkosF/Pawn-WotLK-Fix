...
local function PawnCheckItemTypeCore(ItemLink, AllowEquippable, AllowStatGems, AllowRelics)
	...
	elseif ((not VgerCore.RangedSlotExists) and (InvType == "INVTYPE_RELIC" or InvType == "INVTYPE_THROWN")) or InvType == "INVTYPE_TABARD" or InvType == "INVTYPE_BAG" or InvType == "INVTYPE_BODY" then
		-- Thrown items, tabards, bags, and shirts (invtype_body) can never have stats.
		return false
	end
...
function PawnCanItemHaveStats(ItemLink)
	return PawnCheckItemTypeCore(ItemLink, true, true, false)
end

-- function PawnCanItemBeArtifactUpgrade(ItemLink)
-- 	return PawnCheckItemTypeCore(ItemLink, false, false, true) -- NOT USED IN 3.3.5
-- end
...
function PawnGetItemData(ItemLink)
	...
	local ItemName, NewItemLink, ItemRarity, ItemLevel, _, _, _, _, InvType, ItemTexture, _, ItemID = GetItemInfo(ItemLink)
	...
	if not Item then
		...
		Item.Level = ItemLevel -- The level from GetItemInfo doesn't take into effect upgrades or heirloom scaling
		Item.ID = ItemID
		...
		if PawnNeverUsableStats[ClassID] then
...
if not ITEM_LEVEL then ITEM_LEVEL = "Item Level %d" end
if not ITEM_LEVEL_PLUS then ITEM_LEVEL_PLUS = "Item Level %d+" end
